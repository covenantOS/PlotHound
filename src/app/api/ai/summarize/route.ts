...
import { buildSummarizeAncestorPrompt } from '@/lib/ai/prompts'
  ...
  try {
    ...
    if (!canAccessAiFeatures(profile?.subscription_tier || 'free')) {
      return NextResponse.json(
        { error: 'Upgrade to Researcher to access AI summaries' },
        { status: 403 }
      )
    }
    ...
    const prompt = buildSummarizeAncestorPrompt(ancestorWithContext)
